  ...
  <div class="fixed top-4 right-4 z-50 space-y-2 max-w-sm w-full">
    <TransitionGroup name="toast">
      ...
      >
        ...
        <!-- Message -->
        <p class="flex-1 text-sm text-gray-700 font-medium leading-snug px-3 py-3">{{ toast.message }}</p>
        <!-- Dismiss -->
        <button @click="remove(toast.id)" class="text-gray-300 hover:text-gray-500 transition shrink-0 pr-3">
          <X class="w-3.5 h-3.5" />
        </button>
      </div>
    </TransitionGroup>
