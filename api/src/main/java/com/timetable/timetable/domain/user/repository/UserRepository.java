...
public interface UserRepository extends JpaRepository<ApplicationUser, Long> {
    ...
    Page<ApplicationUser> findAllExcludingRole(@Param("role") com.timetable.timetable.domain.user.entity.UserRole role, Pageable pageable);

    @Query("SELECT u FROM ApplicationUser u WHERE :role NOT IN (SELECT r.role FROM u.roles r)")
    List<ApplicationUser> findAllExcludingRole(@Param("role") com.timetable.timetable.domain.user.entity.UserRole role);

    default Page<ApplicationUser> findAllNonAdminUsers(Pageable pageable) {
        return findAllExcludingRole(com.timetable.timetable.domain.user.entity.UserRole.ADMIN, pageable);
    }
    default List<ApplicationUser> findAllNonAdminUsers() {
        return findAllExcludingRole(com.timetable.timetable.domain.user.entity.UserRole.ADMIN);
    }

    @Query("SELECT u FROM ApplicationUser u JOIN u.roles r WHERE r.role = :role")
    List<ApplicationUser> findAllByRole(@Param("role") com.timetable.timetable.domain.user.entity.UserRole role);
